		       ...
		       enum virtchnl_ops op_to_poll)
{
	...
	while (1) {
		 ...
		 * IAVF_ERR_ADMIN_QUEUE_NO_WORK. Loop few times in case the VF
		 * side is faster or the PF is not responding fast enough.
		 */
		status = iavf_clean_arq_element(hw, event, NULL);
		if (status == IAVF_ERR_ADMIN_QUEUE_NO_WORK && wait--) {
			msleep(IAVF_POLL_WAIT_MS);
			continue;
		}
...
void iavf_configure_queues(struct iavf_adapter *adapter)
{
	...
	while (rem > 0) {
		...
		len = virtchnl_ss_vsi_queue_config_info(vqci, qpair, pairs);
...
void iavf_map_queues(struct iavf_adapter *adapter)
{
	...
	len = virtchnl_ss_irq_map_info(vimi, vecmap, adapter->num_msix_vectors);
	...
	vecmap = vimi->vecmap;
	/* Queue vectors first */
	for (v_idx = 0; v_idx < q_vectors; v_idx++, vecmap++) {
		unsigned long map;
...
{
	bool more = false, is_primary = false;
	struct virtchnl_ether_addr_list *veal;
	int len, i = 0, count = 0;
	struct iavf_mac_filter *f;

	veal = kzalloc(IAVF_MAX_AQ_BUF_SIZE, GFP_KERNEL);
	if (!veal)
		return;
	...
	len = virtchnl_ss_ether_addr_list(veal, list, count);
...
out:
	...
	if (count)
		iavf_send_pf_msg(adapter, VIRTCHNL_OP_ADD_ETH_ADDR, (u8 *)veal,
				 len);
	kfree(veal);
}
...
out:
	spin_unlock_bh(&adapter->mac_vlan_list_lock);
	if (count)
		iavf_send_pf_msg(adapter, VIRTCHNL_OP_DEL_ETH_ADDR, (u8 *)veal,
				 len);
	kfree(veal);
}
...
void iavf_add_vlans(struct iavf_adapter *adapter)
{
	...
	vcmsg = kzalloc(IAVF_MAX_AQ_BUF_SIZE, GFP_KERNEL);
	if (!vcmsg)
		return;
	...
	if (VLAN_ALLOWED(adapter)) {
		struct virtchnl_vlan_filter_list *vvfl = vcmsg;

		op = VIRTCHNL_OP_ADD_VLAN;
		len = virtchnl_ss_vlan_filter_list(vvfl, vlan_id, count);
		if (len > IAVF_MAX_AQ_BUF_SIZE) {
			dev_warn(&adapter->pdev->dev, "Too many add VLAN changes in one request (A)\n");
			while (len > IAVF_MAX_AQ_BUF_SIZE)
				len = virtchnl_ss_vlan_filter_list(vvfl, vlan_id,
								   --count);
			more = true;
		}
		...
		struct virtchnl_vlan_filter_list_v2 *vvfl_v2 = vcmsg;
		...
		op = VIRTCHNL_OP_ADD_VLAN_V2;
		len = virtchnl_ss_vlan_filter_list_v2(vvfl_v2, filters, count);
		if (len > IAVF_MAX_AQ_BUF_SIZE) {
			dev_warn(&adapter->pdev->dev, "Too many add VLAN changes in one request (B)\n");
			while (len > IAVF_MAX_AQ_BUF_SIZE)
				len = virtchnl_ss_vlan_filter_list_v2(vvfl_v2,
								      filters,
								      --count);
			more = true;
		}
...
out:
	spin_unlock_bh(&adapter->mac_vlan_list_lock);
	if (count)
		iavf_send_pf_msg(adapter, op, vcmsg, len);
	kfree(vcmsg);
}
...
void iavf_del_vlans(struct iavf_adapter *adapter)
{
	...
	list_for_each_entry_safe(f, ftmp, &adapter->vlan_filter_list, list) {
		/* if VLAN capabilities are not allowed, we dont want to send
	...
	if (VLAN_ALLOWED(adapter)) {
		...
		op = VIRTCHNL_OP_DEL_VLAN;

		len = virtchnl_ss_vlan_filter_list(vvfl, vlan_id, count);
		if (len > IAVF_MAX_AQ_BUF_SIZE) {
			dev_warn(&adapter->pdev->dev, "Too many delete VLAN changes in one request (A)\n");
			while (len > IAVF_MAX_AQ_BUF_SIZE)
				len = virtchnl_ss_vlan_filter_list(vvfl, vlan_id,
								   --count);
			more = true;
...
void iavf_set_rss_key(struct iavf_adapter *adapter)
{
	...
	len = virtchnl_ss_rss_key(vrk, key, adapter->rss_key_size);
...
void iavf_set_rss_lut(struct iavf_adapter *adapter)
{
	...
	len = virtchnl_ss_rss_lut(vrl, lut, adapter->rss_lut_size);
...
void iavf_send_vc_msg(struct iavf_adapter *adapter)
{
	...
	mutex_lock(&adapter->vc_msg_queue.lock);
...
out_unlock:
	mutex_unlock(&adapter->vc_msg_queue.lock);
}
...
void iavf_enable_channels(struct iavf_adapter *adapter)
{
	...
	len = virtchnl_ss_tc_info(vti, list, adapter->num_tc);
			      ...
			      u8 *msg, u16 msglen)
{
	...
	if (v_retval) {
		...
		case VIRTCHNL_OP_ADD_VLAN_V2:
			...
			break;
		default:
			dev_err(&adapter->pdev->dev, "PF returned error %d (%s) to our request %d (%s)\n",
				v_retval, virtchnl_stat_str(v_retval),
				v_opcode, virtchnl_op_str(v_opcode));
